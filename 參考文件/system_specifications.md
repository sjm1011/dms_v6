...
文件清單隱藏獨立的「文件編號」欄位，並將文件編號作為文件名稱的顯示前綴，以「文件編號 + 半形空格 + 文件名稱」顯示。文件編號留空時只顯示文件名稱，不得加入替代文字或多餘空白；資料夾名稱維持原值。此規則只影響清單呈現，文件編號仍須以 `dms_doc.dd_code` 獨立實體欄位保存，不得併入 `dd_title`。一般清單、搜尋結果、主文件、相關文件及預約版本均採相同呈現方式。
