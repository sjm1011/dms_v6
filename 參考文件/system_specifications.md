...
1. 文件編號可填寫或留空；有填寫時，前端必須去除前後空白並轉為大寫。
2. 建立文件主檔。
3. 上傳第一個正式發佈檔案。
4. 填寫修訂日期、生效日期，並選取或輸入異動說明。
   - 生效日期預設為當下日期。
5. 版本號可填寫或留空。
6. 文件主檔狀態設為 `Effective` (有效的)。
7. 第一個文件版本寫入 `effective_at` (生效時間)。
8. 第一個文件版本的 `effective_until` (結束時間) 保持空白。
9. 若第一個版本尚未到生效日期，一般使用者不可看到該文件；系統管理員與對文件所屬資料夾具有效管理權的資料夾管理員可看到此尚未生效版本。
10. 管理員查看尚未生效文件時，文件清單的「屬性 / 版本」欄位顯示「即將生效」；滑鼠移至該狀態時，顯示生效日期。第一版尚未生效時，清單只顯示該預約版本。
