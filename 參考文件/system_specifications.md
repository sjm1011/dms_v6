...
* 新增資料夾預設為 `access_type = 2`。若未設定授權部門或授權使用者，則只有 ADMIN 與有效資料夾管理員可見。
* **限閱屬性繼承規則**：直接上層資料夾為限閱時，其子資料夾必須繼承上層資料夾的限閱屬性，不得再進行其他限閱的設定。
* **目前資料夾屬性顯示**：進入實體資料夾後，畫面標題與項目數量之間必須顯示「公開」或「限閱」文字框。具備該資料夾管理權限的使用者可點擊非繼承屬性的文字框開啟資料夾屬性 Modal。繼承上層限閱屬性的文字框不得提供點擊功能，只能以提示文字顯示實際授權內容，且不得覆寫繼承設定。
* **限閱授權狀態辨識**：目前資料夾標題列及清單「屬性／版本」欄位內的「限閱」文字框，都必須以圖示區分「未設定詳細授權」與「已授權」，不得只以顏色表達狀態。未設定詳細授權時顯示警示圖示；已有有效部門或特定使用者授權時顯示完成圖示，並以提示文字提供授權摘要。
