...
* **管理員身分資訊最小化**：
  - 資料夾樹、資料夾清單、搜尋結果及文件清單均不得顯示或夾帶資料夾管理員身分資訊。
...
若正式發佈檔案不是 PDF (可攜式文件格式)，例如 Word (文字文件)、Excel (試算表)、PowerPoint (簡報) 或圖檔：
...
* `GET /api/folders`、`GET /api/folders/acl`、文件清單與搜尋 API 的一般回應，不得包含資料夾管理員姓名、員工編號、登入帳號、直接管理員清單、繼承管理員清單或有效管理員清單。
